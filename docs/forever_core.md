...
| Supported Paladin talent effects, offensive abilities, self buffs and native APL | `classic60_paladin_build_test.go`, `classic60_paladin_abilities_test.go`, `classic60_paladin_buffs_test.go` |
...
The Mage reference accepts only unmodified positive integer flat costs. The Paladin extension below also preserves unmodified base-mana percentage costs; reductions and regeneration talents remain unsupported. Interrupted hardcasts neither spend mana nor refresh the five-second rule; completed casts spend their cost even if the projectile subsequently misses. Instant costs refresh the rule immediately, and a tick exactly at expiry uses full regeneration.
...
This is the first real spell rotation through the modern engine under bounded Classic rules, not a complete Mage simulator or verified Forever behavior. The source caveats about spell crit suppression and resistance remain. Development now prioritizes Paladin, beginning with Retribution. Mage remains a regression reference. Incoming/front-facing combat, forms and pets still need their own acceptance coverage.

### Classic 60 Paladin Command integration

`classic60PaladinReferenceRules` combines the audited melee, spell and mana policies for a private pre-racial Human Paladin. This first Retribution slice uses an equipped two-handed sword, mace or axe from behind a passive level 60–63 enemy. A private spell tag admits only the two Holy Command attacks into melee outcomes; ordinary white swings retain physical armor, glancing and weapon-skill behavior. The public ruleset remains TBC and the deployed preview remains the stat diagnostic.

The implementation follows executable [Seal of Command](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/paladin/soc.go), [Judgement](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/paladin/judgement.go), and [Paladin mana regeneration](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/paladin/paladin.go) at the same pinned Classic revision as the core ledger. The original untalented fixture establishes these values:

| Component | Supported source behavior |
| --- | --- |
...
| Seal of Command rank 5 (20920) | Explicitly learned, 210 mana, 1.5-second GCD, 30-second aura. |
| Command proc (20947) | Landed white attacks trigger a 7 PPM roll using base weapon speed, with a one-second shared cooldown. Fresh non-normalized weapon damage including AP, plus `0.29 × spell power`, is multiplied by 0.7. |
| Proc outcome | Independent rear melee-special miss/dodge/crit, no glancing, double-damage melee crits, Holy nonbinary resistance, damage delivered after the source's explicit 10ms delay. |
| Judgement (20271) | Off GCD, ten-second cooldown, six percent of base mana: **90.72**, preserved without integer truncation. Consumes the seal even on a miss. |
| Judgement of Command (20966) | Unstunned base damage 169.5–186.5 plus `0.429 × spell power`; separate magical hit check followed by melee crit only, with Holy nonbinary resistance. |

The shared mana path retains a private floating-point base cost for these Classic profiles while leaving inherited TBC integer arithmetic intact. A separately classified magical hit helper lets Judgement use spell hit and melee crit without sending a Holy melee attack through a generic spell table. Holy has no resistance stat, but the source's level-based partial resistance still applies against higher-level enemies.

The proc's effective spell-power coefficient is **0.203**, matching executable source multiplication. This is source parity, not independent client or Forever verification. The pinned source omits the Command talent check; this port explicitly requires the seal to be learned. The supported build below adds selected talent and buff modifiers to this foundation. Racials, haste, stunned targets, alternate seals, weapon swaps, twisting and external item/proc hooks remain excluded. The isolated 10ms proc delivery does not establish a general batching model. Seal of Righteousness is deferred because its source contains SoD-derived weapon formulas and ambiguous spell-power scaling.

Six integration tests cover exact costs and FSR/MP5 boundaries, seal expiry and missed-Judgement consumption, literal weapon/SP/crit/resistance damage, white-only PPM and cooldowns, and a depleted-mana native APL rotation with uninterrupted three-second swings. Three-iteration tests verify recovery, resource reports and identical-seed reproducibility. A five-millisecond encounter catches cleanup of a Command result scheduled beyond the encounter boundary. This untalented regression remains alongside the expanded build tests below.

### Classic 60 Paladin talents and offensive abilities

`classic60_paladin_talents.go` defines the complete **44-entry Classic talent schema**, independently of the TBC protobuf: 14 Holy, 15 Protection and 15 Retribution entries. The parser accepts the Classic calculator string format and validates ASCII ranks, maximum ranks, the 51-point budget, points in earlier rows and all five prerequisite arrows against the pinned [talent tree](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/ui/core/talents/trees/paladin.json). Structural validation does not imply implemented combat effects. `registerClassic60PaladinBuild` separately rejects unsupported selections before changing the character; it also requires the private pre-racial Human Paladin profile, rear combat and a two-handed sword, mace or axe.

The legal **11/8/32** build `550001-503-55205051000315` spends all 51 points on supported effects. It combines Divine Strength/Intellect, Consecration, Improved Devotion Aura, Precision, Improved Might, Benediction, Improved Judgement, Deflection, Conviction, Command, Two-Handed Weapon Specialization, Sanctity Aura and Vengeance. Other accepted builds can use Toughness and Blessing of Kings. Deflection supplies its parry percentage and Toughness supplies equipment-armor scaling; neither enables incoming combat. The fixture uses a synthetic weapon, with no gear catalog or gear preset import.

The [talent effects](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/paladin/talents.go) preserve their separate channels: Precision changes physical hit, Conviction changes physical crit, and weapon specialization multiplies physical damage plus the two Command attacks separately. Benediction reduces only the supported seal/Judgement costs by 3% per rank, preserving fractional costs: rank 5 gives **178.5** for Command and **77.112** for Judgement. Improved Judgement reduces the cooldown to eight seconds at rank 2. Any dealt critical result refreshes Vengeance's eight-second Physical/Holy multiplier, up to 15%, without stacking it.

| Added ability | Supported level-60 behavior |
| --- | --- |
| Consecration rank 5 (20924) | Talent required; 565 mana, eight-second cooldown, eight one-second ticks of `48 + 0.042 × spell power` per active target. Snapshots spell power and attacker multipliers; each tick rolls a noncritical magical hit. |
| Exorcism rank 6 (10314) | 345 mana, fifteen-second cooldown, `505–563 + 0.429 × spell power`; Undead/Demon targets only. Preserves the source's binary magical hit/crit classification. |
| Holy Wrath rank 2 (10318) | 805 mana, two-second cast, sixty-second cooldown, `490–576 + 0.19 × spell power` against active Undead/Demon targets. Calculates all target results before delivering proc callbacks. A due white swing waits for the hardcast to finish. |
| Offensive Holy Shock rank 3 (20930) | Talent required; 325 mana, thirty-second cooldown, `365–395 + 0.429 × spell power`; magical hit/crit. Its effect is tested in isolation, not yet accessible through a supported full Holy build. |

These ranks follow the pinned [offensive spell implementations](https://github.com/wowsims/classic/tree/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/paladin). **Holy Power and Improved Blessing of Wisdom also remain isolated effect coverage**: the supported early Holy talents provide at most eleven points, so the higher rows cannot be reached legally yet. Holy Shock additionally requires the unimplemented Divine Favor path. Following the source's outcome code, Holy Power increases Holy magical crit but does not increase the melee-classified Command proc or Judgement crit.

Self blessings use trainer ranks rather than AQ-book upgrades: Might rank 6 grants 155 AP for 110 mana, Wisdom rank 5 grants 30 MP5 for 115 mana, and learned Kings grants 10% primary attributes for 8% base mana (**120.96**). All last five minutes, use the normal GCD and replace another blessing from this same Paladin; refreshes do not stack. Might and Wisdom talent ranks affect their respective bonuses. Buff values and effects come from the pinned [buff implementation](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/sim/core/buffs.go) and [spell tooltip snapshot](https://github.com/wowsims/classic/blob/7779ebbf79dc7f1341e6ab939b28a3402c9a730a/assets/db_inputs/wowhead_spell_tooltips.csv).

Devotion rank 7 supplies 735 armor, increased 5% per Improved Devotion rank; learned Sanctity supplies 10% Holy damage. Exactly one aura, or none, is selected before finalization and restored on reset. No castable aura-switch action is registered: in-combat switching, its GCD and party range are outside this implementation. Blessings are self-only; raid distribution is not modeled.

The private Paladin profile now preserves fractional primary attributes and derived stats during initialization and dynamic buff changes, matching the pinned Classic evaluator. Divine Strength, Divine Intellect and Kings therefore compose without inheriting TBC flooring. For the supported build, tests pin 115.5 Strength, 391 AP and 2,387 total mana while base mana remains 1,512. This remains source-code parity, not independent client confirmation; public TBC rounding and results remain unchanged.

Tests cover schema validation, rejection before mutation, talent costs and channels, Vengeance gain/refresh/expiry, literal spell damage, target restrictions, AoE snapshots, interruption, due-swing deferral, blessing replacement/expiry, aura selection and dynamic Kings dependencies. A ninety-second, three-iteration native APL fight exercises Might, Command/Judgement, Exorcism, Consecration and Holy Wrath, reaches mana recovery and reproduces identical seeded results.

**Remaining Paladin work:** other seals and their judgements; Hammer of Wrath's distinct ranged outcome path; healing and Divine Favor; defensive, threat, shield and incoming-combat mechanics; crowd control and utility abilities; the unimplemented talent effects; racials and the original WoWSims class interface. The pinned Vindication implementation incorrectly buffs the Paladin's own AP and is explicitly rejected. Shield Specialization carries an unresolved SoD-derived block-value caveat. The stale Holy UI preset does not fit its own Classic tree and is rejected by the parser. These gaps are tracked rather than silently filled from TBC. The public ruleset and Pages stat diagnostic remain unchanged, and this is not yet a complete playable Paladin simulator.
