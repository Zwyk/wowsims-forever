...
The active profile still reproduces the inherited TBC engine. Attack tables now also cache explicit armor/resistance model selectors. A deliberately partial internal Classic profile connects characterized components for the scheduled melee fixture below; it cannot be selected through the public API. Talents, class spells, equipment data and UI activation remain separate work. A complete Forever profile will be activated only with matching level-60 data and UI changes.
...
The proc's effective spell-power coefficient is **0.203**, matching executable source multiplication. This is source parity, not independent client or Forever verification. The pinned source omits the Command talent check; this port explicitly requires the seal to be learned. The initial fixture excludes other talents, racials, damage/cost modifiers, haste, stunned targets, alternate seals, weapon swaps, twisting and external item/proc hooks. The isolated 10ms proc delivery does not establish a general batching model. Seal of Righteousness is deferred because its source contains SoD-derived weapon formulas and ambiguous spell-power scaling.

Six integration tests cover exact costs and FSR/MP5 boundaries, seal expiry and missed-Judgement consumption, literal weapon/SP/crit/resistance damage, white-only PPM and cooldowns, and a depleted-mana native APL rotation with uninterrupted three-second swings. Three-iteration tests verify recovery, resource reports and identical-seed reproducibility. A five-millisecond encounter catches cleanup of a Command result scheduled beyond the encounter boundary. The next Paladin milestone is a supported class constructor, level-60 gear and talent inputs, and the original WoWSims simulator interface. Protection additionally requires validated front-facing and incoming combat; the shared seal and mana work can support that later.
