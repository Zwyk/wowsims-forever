...
The executable scope is a pre-racial Human Warrior at level 60, with one explicitly equipped main-hand physical weapon, attacking a passive level 60–63 enemy from behind. The test constructs a real Character and runs normal reset, weapon scheduling, damage application and metrics across iterations. No class agent, talent, racial, item effect, resource bar or rotation is imported from TBC. The live physical wrapper rejects unsupported contexts rather than falling back to inherited tables; the broader pure weapon reference remains available for characterization.
...
The remaining runtime work includes dual wield, specials and incoming combat, resource/regen and haste policies, live racial/final-stat rounding, forms/pets, and matching class/talent/spell/item/encounter data. The fixture's integer pre-racial stats intentionally avoid implying that the modern live flooring behavior has already been converted. Unsupported nonzero defense/resilience conversions fail explicitly; absent zero values no longer produce `0/0` during initialization. The public simulator and browser lab retain their existing scope.
