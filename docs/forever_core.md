...
Only unmodified positive integer flat costs are accepted. Classic's floating-point percentage-cost calculation and modern TBC's integer percentage bucket are not equivalent in general; base-mana percentage costs, reductions and regeneration talents remain an explicit next boundary. Interrupted hardcasts neither spend mana nor refresh the five-second rule; completed casts spend their cost even if the projectile subsequently misses. Instant costs refresh the rule immediately, and a tick exactly at expiry uses full regeneration.
...
This is the first real spell rotation through the modern engine under bounded Classic rules, not a complete Mage simulator or verified Forever behavior. The source caveats about spell crit suppression and resistance remain. Next work should expand this into a coherent minimal Mage class with supported spell ranks, costs/modifiers and matching level-60 gear/talents before exposing it through the original WoWSims interface. Other classes, incoming/front-facing combat, forms and pets still need their own acceptance coverage.
