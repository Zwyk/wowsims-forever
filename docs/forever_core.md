...
- Whether Forever uses Classic or TBC armor and resistance formulas, caps and modifier ordering in every case; the compiled Classic armor projection is only a pinned comparison fallback.
...
Shared Hit/Crit inputs, armor mitigation, and weapon-skill categories and bonuses are now represented by inactive seams in the modern engine. Source-pinned Classic level-60 reference policies and regression vectors characterize possible fallbacks, while Forever units, populated data and live combat effects remain deliberately unresolved.
